...
metadata {
    definition (name: "Somfy MyLink Hub", namespace: "defunctzombie.somfy.mylink", author: "") {
        capability "Actuator"
        capability "Sensor"
        capability "Refresh"
        capability "Polling"
...
// able to keep up with the requests.
...
def createChildDevices() {
        ...
        devices.each {deviceID, deviceName ->
            ...
            if (!deviceExists) {
                ...
                addChildDevice("defunctzombie.somfy.mylink", "Somfy MyLink Shade", deviceID, [
                        "label": "${deviceName}"
                ])
            }
        }
...
def childUp(String deviceID) {
    log.debug("childUp")
    sendCommand("mylink.move.up", deviceID)
}

def childDown(String deviceID) {
    log.debug("childDown")
    sendCommand("mylink.move.down", deviceID)
}

def childStop(String deviceID) {
    log.debug("childStop")
